$(document).ready(function () {
    ...
    $('#editModal').on('show.bs.modal', function (event) {
        ...
        fetch(`${BASE_URL}/admin/services/getService/${serviceId}`)
            ...
            .then(data => {
                ...
                categorySelect.prop('disabled', isCategory);

                if (!isCategory) {
                    categorySelect.val(data.parent_id).trigger('change');
                } else {
                    categorySelect.val('').trigger('change');
                }

                checkBox.off('change').on('change', function () {
                    categorySelect.prop('disabled', this.checked);
    ...
    $('#editForm').on('submit', function (e) {
        ...
        const isCategory = $('#checkBoxCategory').is(':checked') ? 1 : 0;
        const categorySelect = $('.js-example-basic-single2');
        const categoryValue = categorySelect.val() || 0;
