        ...
        <div class="custom-part-with-border mt-4">
                    ...
                    <div class="input-group">
                        ...
                        <?php else:?>
                            <?php if (!empty($search)): ?><button class="btn btn-search border-start-0" type="button" id="btn-delete">
                                <a href="?page=1&per_page=<?= $perPage ?>" class="center text-decoration-none"><i class='fas fa-xmark text-primary'></i></a>
                                </button>
                            <?php endif;?>
                        <?php endif; ?>
                    </div>
            ...
            <div class="d-flex flex-wrap justify-content-between mt-5">
                <div class="d-flex" id="filterLinks">
                    <a href="?filter=all<?= (!empty($search)) ? "&search=" . urlencode($search) : "" ?>" class="text-decoration-none"><?= __("all") ?> (<?=$allVisits ?>)</a>
                    <div class="vertical-separator"></div>
                    <a href="?filter=reserved<?= (!empty($search)) ? "&search=" . urlencode($search) : "" ?>" class="text-decoration-none">
                        <?= __("reserved") ?> (<?=$visitsReserved ?>)
                    </a>
                    <div class="vertical-separator"></div>
                    <a href="?filter=done<?= (!empty($search)) ? "&search=" . urlencode($search) : "" ?>" class="text-decoration-none "><?= __("done") ?> (<?= $allDoneVisitsSize ?>)</a>
                    <div class="vertical-separator"></div>
                    <a href="?filter=cancelled<?= (!empty($search)) ? "&search=" . urlencode($search) : "" ?>" class="text-decoration-none">
                        <?= __("cancelled") ?> (<?= $allCancelledVisitsSize ?>)
                    </a>
                </div>
                <?php if(isset($_GET['search'])): ?>
                    <div><?php printf( __("item"),1) ?></div>
                <?php endif; ?>
            </div>
                        ...
                        <table class="table custom-table">
                            ...
                            <tr>
                                ...
                                <th class="text-center" >
                                    <a href="<?= $sortServiceUrl ?>" class="text-decoration-none text-white d-flex justify-content-center align-items-center">
                                        <?= __('service')?>
                                ...
                                <th >
                                    <a href="<?= $sortEmployeeUrl ?>" class="text-decoration-none text-white d-flex justify-content-center align-items-center">
                                        <?= __('employee')?>
                                ...
                                <th>
                                    <a href="<?= $sortDateUrl ?>" class="text-decoration-none text-white d-flex justify-content-center align-items-center">
                                        <?= __('visit_date')?>
                                        <?php if ($sortBy === 'date'): ?>
                                            <?php if ($sortOrder === 'asc'): ?>
                                                <i class="fa-solid fa-caret-up mx-1"></i>
                                    ...
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content">
                                                ...
                                                <div class="modal-body2">
                                                    ...
                                                    <button class="btn btn-outline-primary px-5 mt-4" style="margin-top: 35px;" data-bs-dismiss="modal"><?= __("close")?></button>
