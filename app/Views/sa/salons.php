...
<div class="card shadow-sm">
    <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
        <h4 class="mb-0"><?= __('salon_list') ?></h4>
        <a href="<?= BASE_URL ?>/sa/salon/create" class="btn btn-light btn-sm">➕ <?= __('add_salon') ?></a>
    </div>
    <div class="card-body">

        <?php if (empty($salons)): ?>
            <div class="alert alert-info"><?= __('no_salons_found') ?></div>
        <?php else: ?>
            <div class="table-responsive">
                <table class="table table-striped table-hover align-middle">
                    ...
                    <?php foreach ($salons as $salon): ?>
                        <tr>
                            <td><?= htmlspecialchars($salon->id) ?></td>
                            <td><?= htmlspecialchars($salon->name) ?></td>
                            <td><?= htmlspecialchars($salon->manager) ?></td>
                            <td><?= htmlspecialchars($salon->manager_mobile) ?></td>
                            <td><a><?= htmlspecialchars($salon->link_name) ?></a></td>

                            <td>
                                    <span class="badge <?= $salon->active ? 'bg-success' : 'bg-danger' ?>">
                                        <?= $salon->active ? __('yes') : __('no') ?>
                                    </span>
                            </td>
                            <td>
                                <a href="<?= BASE_URL ?>/sa/salon/edit/<?= $salon->id ?>" class="btn btn-sm btn-warning">
                                    ✏️ <?= __('edit') ?>
                                </a>

                                <form action="<?= BASE_URL ?>/sa/salon/delete/<?= $salon->id ?>" method="post" class="d-inline"
